...
export default function AdminLayout({ children }) {
    ...
    return (
      <Link 
        ...
        className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group ${
          isActive 
            ? 'bg-[#c71e22] text-white shadow-md shadow-red-200' 
  ...
  return (
    <div className="flex h-screen bg-[#f8f9fb] overflow-hidden font-sans">
      ...
      {/* Sidebar */}
      <aside className={`fixed md:relative z-30 w-[240px] h-full bg-white border-r border-gray-100 flex flex-col flex-shrink-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        {/* Logo Area */}
        <div className="px-5 py-5 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <Image src="/logo@2x.png" alt="Jains Computer" width={140} height={36} className="object-contain" priority />
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5">
          ...
          {/* Manage Section */}
          <div className="mb-5">
            <h3 className="px-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">Manage</h3>
          ...
          {/* Account Section */}
          <div className="mb-5">
            <h3 className="px-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">Account</h3>
            <div className="space-y-1.5">
        ...
        <div className="p-4 border-t border-gray-100">
          <button 
            ...
            className="flex items-center w-full px-4 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-red-50 hover:text-red-600 transition-all duration-200 group"
          >
      ...
      <main className="flex-1 flex flex-col h-full relative min-w-0">
        {/* Header */}
        <header className="h-16 border-b border-gray-100 flex items-center justify-between px-4 md:px-6 bg-white flex-shrink-0 min-w-0">
          ...
          <div className="flex items-center gap-4 flex-shrink-0 ml-2">
            ...
            <div className="flex items-center gap-2.5 cursor-pointer">
              <div className="w-9 h-9 rounded-full bg-[#c71e22] text-white font-bold flex items-center justify-center text-sm flex-shrink-0">A</div>
        ...
        {/* Dynamic Content */}
        <div className="flex-1 overflow-auto p-3 sm:p-5 md:p-6 relative min-w-0">
          {children}
        </div>

        {/* Footer */}
        <footer className="py-3 border-t border-gray-100 flex flex-col md:flex-row items-center justify-center md:justify-between px-4 md:px-6 text-xs text-gray-400 font-medium bg-white flex-shrink-0 gap-1 text-center">
          <span>© 2026 Jains Computer. All rights reserved.</span>
