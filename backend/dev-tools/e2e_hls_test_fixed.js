...
async function run() {
  ...
  // Create token with correct payload: userId instead of id!
  const token = jwt.sign({ userId: student._id, role: "student" }, process.env.JWT_SECRET || "default_jwt_secret", { expiresIn: "1d" });
  ...
  if (masterRes.status !== 200) {
      console.log("Master Failed:", masterBody);
      process.exit(1);
  }
  ...
  console.log(`GET ${segmentUrl.substring(0, 80)}...`);
